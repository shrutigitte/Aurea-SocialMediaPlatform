...
export default function PostDetailPageClient({ postId }: { postId: number }) {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        ...
        (p) => {
          setLikeCount((c) => c + 1);
          if ((p.new as any).user_id === me) setLikedByMe(true);
        }
      )
      .on(
        "postgres_changes",
        ...
        (p) => {
          ...
          if ((p.old as any).user_id === me) setLikedByMe(false);
        }
      )
      .on(
        "postgres_changes",
  ...
  async function toggleLike() {
    ...
    setLikedByMe(!curr);
    setLikeCount((c) => Math.max(0, c + (curr ? -1 : 1)));
    ...
    return (
      <main className="max-w-2xl mx-auto p-4">
        <div className="rounded-xl border bg-white p-6 text-sm text-gray-600">
          ...
          <Link className="text-purple-600 underline" href="/feed">
            Go back
          </Link>
        </div>
      </main>
    ...
    <main className="max-w-2xl mx-auto p-4">
      <article className="p-4 border rounded-xl bg-white shadow">
        ...
        <div className="mt-3 flex items-center flex-wrap gap-2">
          ...
          <button onClick={copyLink} className="text-xs px-3 py-1 rounded border border-gray-200 hover:bg-gray-50">
            Copy link
          </button>
